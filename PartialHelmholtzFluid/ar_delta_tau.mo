...
function ar_delta_tau "residual part of dimensionless Helmholtz energy"

  input Real delta;
  input Real tau;
  output Real alpha_residual_delta_tau
...
algorithm
  alpha_residual_delta_tau := sum(n[i]*d[i]*t[i]*delta^(d[i] - 1)*tau^(t[i] - 1) for i in 1:7)
    + sum(n[i]*t[i]*delta^(d[i] - 1)*tau^(t[i] - 1)*(d[i] - c[i]*delta^c[i])*exp(-delta^c[i]) for i in 8:23)
    + sum(n[i]*delta^d[i]*tau^t[i]*exp(-eta[i]*(delta - epsilon[i])^2 - beta[i]*(tau - gamma[i])^2)*(d[i]/delta - 2*eta[i]*(delta - epsilon[i]))*(t[i]/tau - 2*beta[i]*(tau - gamma[i])) for i in 24:25);
