...
algorithm
  alpha_residual_tau := sum(n[i]*t[i]*delta^d[i]*tau^(t[i] - 1) for i in 1:7)
    + sum(n[i]*t[i]*delta^d[i]*tau^(t[i] - 1)*exp(-delta^c[i]) for i in 8:23)
    + sum(n[i]*delta^d[i]*tau^t[i]*exp(-eta[i]*(delta - epsilon[i])^2 - beta[i]*(tau - gamma[i])^2)*(t[i]/tau - 2*beta[i]*(tau - gamma[i])) for i in 24:25);
