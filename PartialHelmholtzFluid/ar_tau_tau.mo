...
algorithm
  alpha_residual_tau_tau := sum(n[i]*t[i]*(t[i] - 1)*delta^d[i]*tau^(t[i] - 2) for i in 1:7)
    + sum(n[i]*t[i]*(t[i] - 1)*delta^d[i]*tau^(t[i] - 2)*exp(-delta^c[i]) for i in 8:23)
    + sum(n[i]*delta^d[i]*tau^t[i]*exp(-eta[i]*(delta - epsilon[i])^2 - beta[i]*(tau - gamma[i])^2)*((t[i]/tau - 2*beta[i]*(tau - gamma[i]))^2 - t[i]/tau^2 - 2*beta[i]) for i in 24:25);
