...
algorithm
      ...
      while (abs(R4/s)>tolerance) and (iter<iter_max) and (abs(T1-T2)>tolerance) loop
        ...
        if (R4*R3<=0) then
          ...
          elseif (R4*R2<0) then
            ...
            assert(false, "Ancillary.saturationTemperature_s (liquid side): this should not happen. Input was s=" + String(s));
          end if;
        end if;
  ...
  elseif (s<=sl_trip) then
    ...
    assert(false, "Ancillary.saturationTemperature_s (liquid side): this should also not happen. Input was s=" + String(s));
  end if;
