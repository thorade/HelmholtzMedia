...
protected
  ...
  Real a = 0.42747*R_s^2*T_crit^2/p_crit*(1 + m*(1 - sqrt(T/T_crit)))^2;
  Real b = 0.08664*R_s*T_crit/p_crit;
  Real A = a*p/(R_s^2*T^2);
  Real B = b*p/(R_s*T);
  Real r = (A-B-B^2)-1/3;
...
algorithm
  ...
  elseif ((abs(D) < 1e-8) and (abs(r) < 1e-3)) then
    ...
    if (T <= T_crit) then
      ...
      // Modelica.Utilities.Streams.print("d(Y1)=" + String(p/(R_s*T*(Y1+1/3))) + ", d(Y2)=" + String(p/(R_s*T*(Y2+1/3))) + ", d(p/(R_s*T*(Y3+1/3)))=" + String(Y3), "printlog.txt");
      if (p > psat) then
      ...
      // Modelica.Utilities.Streams.print("d(Y1)=" + String(p/(R_s*T*(Y1+1/3))) + ", d(Y2)=" + String(p/(R_s*T*(Y2+1/3))) + ", d(Y3)=" + String(p/(R_s*T*(Y3+1/3))), "printlog.txt");
      d := max({p/(R_s*T*(Y1+1/3)), p/(R_s*T*(Y2+1/3)), p/(R_s*T*(Y3+1/3))});
    end if;
  end if;
